...
class BoletaFacturaXML:
    ...
    def generate_invoice_xml(self):
        ...
        cbc_TaxableAmount.text = str(self.ventas.idPedido.subtotal) #Sum. valor venta (gravadas) subtotal
        ...
        cbc_LineExtensionAmount.text = str(self.ventas.idPedido.subtotal) #Total valor venta
        ...
        cbc_TaxInclusiveAmount.text = str(self.ventas.idPedido.subtotal + self.ventas.idPedido.igv_total) #Total precio venta
        ...
        cbc_PayableAmount.text = str(self.ventas.idPedido.subtotal + self.ventas.idPedido.igv_total) #Importe total de la venta
        ...
        for i in range(len(self.detalle_pedido)):
            ...
            #cbc_InvoicedQuantity.set("unitCodeListID","UN/ECE rec 20")
            #cbc_InvoicedQuantity.set("unitCodeListAgencyName","United Nations Economic Commission for Europe")
            ...
            #cbc_TaxExemptionReasonCode.set("listName","Afectacion del IGV")
            #cbc_TaxExemptionReasonCode.set("listURI","urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo07")
            #cbc_TaxExemptionReasonCode.set("listAgencyName","PE:SUNAT")
            cbc_TaxExemptionReasonCode.text = afectacion_igv #Tipo de Afectacion IGV: 10, 20 (Gravado, Operación Onerosa - Catálogo 07)
            cac_TaxScheme = etree.SubElement(cac_TaxCategory,"{{{cac}}}TaxScheme".format(cac=self.namespaces['cac']))
            cbc_ID = etree.SubElement(cac_TaxScheme,"{{{cbc}}}ID".format(cbc=self.namespaces['cbc']))
            #cbc_ID.set("schemeName","Codigo de tributos")
            #cbc_ID.set("schemeAgencyName","PE:SUNAT")
            #cbc_ID.set("schemeURI","urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo05")
