...
class FirmaDigital:
  ...
  <ds:Reference URI="">
    ...
    <ds:DigestValue>{valor_resumen}</ds:DigestValue>
        ...
        with open('comprobantes/xml/signed-info.xml', 'w', encoding='utf-8') as output_file:
            output_file.write(signed_info_content)

        # Comando de OpenSSL para firmar con SHA-1 y RSA - FIRMAR XML
        openssl_command = ['openssl', 'dgst', '-sha1', '-sign', self.private_key_path, 'comprobantes/xml/signed-info.xml']
...
<ds:KeyInfo>
    <ds:X509Data>
        ...
        xml_path = 'comprobantes/xml/invoice.xml'
       ...
       # Crear un nuevo elemento para signed_info_content y añadirlo a <ext:ExtensionContent>
        ...
        tree.write('comprobantes/xml/invoice-signed.xml', encoding='utf-8', xml_declaration=True)

        #return firma_base64
    
    def calcular_valor_resumen(self):

        with open('comprobantes/xml/invoice.xml', 'rb') as f:
            tree = etree.parse(f)

        canonizado = etree.tostring(tree, method='c14n')

        canonizado_sin_comentarios = b'\n'.join([line for line in canonizado.split(b'\n') if not line.strip().startswith(b'<!--')])
        with open('comprobantes/xml/invoice_c14.xml', 'wb') as f:
           f.write(canonizado_sin_comentarios) # guardar el canonico inicial
                
        openssl_command = ['openssl', 'dgst', '-sha1', '-binary', 'comprobantes/xml/invoice_c14.xml'] # obtener el sha1 del archivo canonico
