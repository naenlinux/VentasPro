              ...
              <table class="table table-bordered table-sm table-striped">
                  ...
                  <tbody>
                     <tr v-for="(ve, index) in arrayVentas" :key="ve.id">
                        <td>{{index+1}}</td>
                        <td class="text-center" :class="ve.activo==0 ? 'text-danger' : 'text-success'" >
                            <div v-if="ve.activo"><strong>Activo</strong></div>
                            <div v-else><strong>Anulado</strong></div>
                        </td>
                        <td class="text-center"><button class="btn btn-sm btn-info" @click="verDetalle(ve.id)"><i class="fas fa-eye"></i></button></td>
                        ...
                        <td class="text-center">
                            <button v-if="ve.nombre_xmlzip" class="btn btn-sm btn-success tooltip-btn" data-tooltip="Descargar" @click="descargar_xml(ve.nombre_xmlzip)">XML <i class="fas fa-file"></i></button>
                            <button v-else class="btn btn-sm btn-danger" @click="generar_xml(ve.id)">Generar</button>
                        </td>
                        <td class="text-center">
                            <button v-if="ve.nombre_cdr" class="btn btn-sm btn-primary tooltip-btn" data-tooltip="Descargar" @click="descargar_cdr(ve.nombre_cdr)">CDR <i class="fas fa-file"></i></button>
                            <button v-else class="btn btn-sm btn-danger" @click="enviarSunat(ve.nombre_xmlzip)">Enviar</button>
                        </td>
                     </tr>
                  </tbody>
              </table>
    ...
    export default{
      ...
      methods:{
        ...
        enviarSunat(nombre_xmlzip){
            ...
            let me = this
            me.$axios.get(`${process.env.VUE_APP_API_URL}/apisunat/sunat/?nombre_xmlzip=${nombre_xmlzip}&enviar_sunat=1`)
            .then(function (response) {
        ...
        generar_xml(id){
            let me = this
            me.$axios.get(`${process.env.VUE_APP_API_URL}/apisunat/sunat/?id_venta=${id}&generar_xml=1`)
            .then(function (response) {
        ...
        descargar_xml(nombre_zip){
            let me = this
            me.$axios.get(`${process.env.VUE_APP_API_URL}/apisunat/sunat/?nombre_zip=${nombre_zip}`,{
                responseType: 'blob'
            })
            .then(function (response) {
        ...
        descargar_cdr(nombre_cdr){
            let me = this
            me.$axios.get(`${process.env.VUE_APP_API_URL}/apisunat/sunat/?nombre_cdr=${nombre_cdr}`,{
                responseType: 'blob'
            })
            .then(function (response) {
                console.log(response)
